        ...
        for d in daily:
            ...
            ForecastDay.objects.update_or_create(
                ...
                defaults={
                    ...
                    'rain_prob':      d['rain_prob'] or 0,
                    'rain_amount':    d['rain_amount'] or 0,
                    'snow_amount':    d['snow_amount'] or 0,
                    'wind_speed':     d['wind_speed'] or 0,
                    'wind_gust':      d['wind_gust'] or 0,
                    'wind_direction': d['wind_direction'] or 0,
                    'humidity':       d['humidity'],       # nullable — Open-Meteo doesn't provide this daily
                    'dew_point':      d['dew_point'],
                    'uv_index':       d['uv_index'] or 0,
                    'pressure':       d['pressure'],       # nullable — Open-Meteo doesn't provide this daily
                    'weather_code':   d['weather_code'],
                    'description':    d['description'],
                    'icon':           d['icon'],
                    'sunrise': timezone.datetime.fromtimestamp(d['sunrise'], tz=timezone.utc) if d.get('sunrise') else None,
                    'sunset':  timezone.datetime.fromtimestamp(d['sunset'],  tz=timezone.utc) if d.get('sunset')  else None,
                }
            )
